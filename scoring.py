...
def get_decision_score(data, fund_data, inst_data=None, mode="post", with_reason=True):
    ...
    if data.get('訊號', False): 
        if is_trending:
            ...
            add(1, f"⚠️ 穩在月線上 (但 ADX:{adx:.1f} 盤整區間，動能稍弱)")
    ...
    # 近月位階
    if roc_20 > 10:
        add(2, f"🔥 近月漲幅 {roc_20:.2f}% 表現亮眼，具備市場主流強勢股特徵")
    elif roc_20 < -5:
        add(-2, f"🩸 近月跌幅 {roc_20:.2f}% 表現弱勢，請避開弱勢接刀陷阱")
    
    # 基本面營收
    if data.get('MoM', 0) > 0 and data.get('YoY', 0) > 0:
        add(3, f"🔥 月營收雙增 (MoM: {data.get('MoM')}%, YoY: {data.get('YoY')}%)，具備長線黑馬特質")
    elif data.get('YoY', 0) > 15:
        add(2, f"✅ 月營收年增達 {data.get('YoY')}%，營運動能強勁")
    ...
    if data.get('成交量', 0) > data.get('5日均量', 0) * 1.1: 
        add(2, "✅ 量能放大 (具備主力進場點火特徵)")
    else: 
        add(-1, "⚠️ 量能未明顯放大 (打底或缺乏點火動能)")
        
    if data.get('MACD柱', 0) > data.get('前日MACD柱', -999): 
        add(2, "✅ MACD 綠柱收斂或紅柱放大 (動能防禦過關)")
    else: 
        add(macd_penalty, "⚠️ MACD 空方動能持續擴大 (型態脆弱嚴防接刀)")

    # K線型態
    if data.get('紅吞', False): 
        add(4 if is_trending else 1, "🔥 出現「紅吞」反轉型態 (趨勢確認，強烈買訊)")
            
    if data.get('黑吞', False): 
        add(-3, "🩸 出現「黑吞」反轉型態 (強烈空頭逃命訊號)")

    if data.get('回測有撐', False): 
        add(2, "🔥 帶量長下影線 (主力回測支撐成功)")
    if data.get('反彈遇壓', False): 
        add(-2, "🩸 反彈遇均線壓力留長上影線 (空方壓制)")
    
    if data.get('收盤價', 0) >= data.get('5MA', 0) and data.get('5日線即將上彎', False): 
        add(1, "🔥 5日線扣低值 (短均線準備上彎發散，短線動能轉強)")
    if data.get('收盤價', 0) < data.get('5MA', 0) and not data.get('5日線即將上彎', False): 
        add(-1, "⚠️ 5日線扣高值 (短均線即遇蓋頭壓力)")

    # 扣分項目
    if data.get('J值', 50) >= 80: 
        add(-3, "⚠️ KDJ高檔過熱")
    if data.get('收盤價', 0) >= data.get('BB_UP', 9999) * 0.98: 
        add(-2, "⚠️ 觸及布林上軌壓力")
    if data.get('BIAS', 0) > 7: 
        add(-2, "⚠️ 正乖離過大")
    if data.get('收盤價', 0) < data.get('20MA', 0): 
        add(-2, "⚠️ 跌破月線支撐")
