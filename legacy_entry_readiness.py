# Frozen from entry_readiness.py at 027f459fdea2a79d04c77979f74c0b936cdbe370 (2026-09-09).
# Do not silently synchronize these strategy functions with the current rules.
...
@no_type_check  # Preserve historical logic, including its original local typing.
...
# The strategy bodies above are copied unchanged from the source revision;
# only a typing decorator is added. The public wrapper validates availability
# and freezes intraday levels.
LEGACY_RULE_DATE = "2026-09-09"
LEGACY_RULE_COMMIT = "027f459fdea2a79d04c77979f74c0b936cdbe370"
LEGACY_RULE_VERSION = "legacy_2026_09_09"


def _annotate_legacy_plan(plan: Mapping[str, Any]) -> dict[str, Any]:
    return {
        **dict(plan),
        "Legacy_Rule_Date": LEGACY_RULE_DATE,
        "Legacy_Rule_Commit": LEGACY_RULE_COMMIT,
        "Legacy_Rule_Version": LEGACY_RULE_VERSION,
    }


def _unavailable_legacy_plan(reason: str) -> dict[str, Any]:
    return _annotate_legacy_plan(_result(LEGACY_STATUS, "watch", reason))


def build_legacy_entry_plan(
    record: Mapping[str, Any], *, intraday: bool = False,
) -> dict[str, Any]:
    """Evaluate the frozen September 9 entry rules, never the current rules.

    A missing decision input is not evidence that its historical condition
    passed.  This availability guard does not add modern financial, sample-size
    or cost-adjusted reward/risk thresholds to the legacy strategy.
    """
    numeric_inputs: tuple[str, ...] = (
        "Score", "收盤價", "Confidence", "RSI", "BIAS", "BB_UP", "漲跌幅", "Est_Vol_Ratio",
    )
    if not intraday:
        numeric_inputs += ("20MA", "ATR", "最高價")
    missing = [
        field for field in numeric_inputs
        if isinstance(record.get(field), bool) or _number(record.get(field)) is None
    ]
    for field in ("Volume_Confirmed", "Signal_Conflict", "Entry_Pattern"):
        if field not in record or record.get(field) is None or str(record.get(field)).strip() == "":
            missing.append(field)
    if missing:
        return _unavailable_legacy_plan("舊制必要資料缺失：" + "、".join(missing) + "。")
    if any((_number(record.get(field)) or 0) <= 0 for field in ("收盤價", "BB_UP")):
        return _unavailable_legacy_plan("舊制必要價格資料無效，暫不判定可執行。")

    baseline = None
    if intraday:
        if record.get("Intraday_Quote_Status") != "realtime":
            return _unavailable_legacy_plan("舊制盤中比較缺少有效即時報價。")
        candidate = record.get("Legacy_Entry_Plan")
        if not isinstance(candidate, Mapping) or candidate.get("Legacy_Rule_Commit") != LEGACY_RULE_COMMIT:
            return _unavailable_legacy_plan("尚未保存舊制盤後計畫，不能重建盤中進場區間。")
        low = _number(candidate.get("Entry_Low"))
        high = _number(candidate.get("Entry_High"))
        stop = _number(candidate.get("Entry_Stop"))
        target = _number(candidate.get("Entry_Target"))
        no_chase = _number(candidate.get("No_Chase_Price"))
        if (
            low is None or high is None or stop is None or target is None or no_chase is None
            or not 0 < stop < low <= high < target or no_chase < high
        ):
            return _unavailable_legacy_plan("舊制盤後價格計畫不完整，不能認定盤中可執行。")
        baseline = candidate
    return _annotate_legacy_plan(build_entry_readiness(
        record, intraday=intraday, baseline_plan=baseline,
    ))
