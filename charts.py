# charts.py - AI機構級強化版圖表模組（主力成本/扣抵值/防亂跑鎖定）
...
def draw_professional_chart(df, latest_price, view_days=120, is_light_mode=False, show_buy_signal=True, show_sup_res=True, show_signals=True, buy_dates=None):
    ...
    # 📌 鎖定左上角顯示最新均線數值 (xref/yref domain 確保不會隨拖曳移動)
    ...
    fig.add_annotation(
        ...
        text=ma_text, showarrow=False, xanchor='left', yanchor='top',
    ...
    fig.add_annotation(
        ...
        text=f"機構趨勢: {trend}", showarrow=False, xanchor='right', yanchor='top',
    ...
    if show_sup_res:
        ...
        avg_s = np.mean(strengths) if strengths else 0
        
        # 畫支撐壓力線
        for lvl, s in zip(levels, strengths):
            width = 1 if s < avg_s else 3
            fig.add_hline(y=lvl, line_dash="dot", line_width=width, line_color="#c084fc", opacity=0.6, row=1, col=1)

        # 畫突破訊號 (回踩支撐/跌破壓力)
        signals = detect_breakout(df_view, levels)
        buy_x, buy_y, sell_x, sell_y = [], [], [], []
        for typ, x, y in signals:
            if typ == "buy": buy_x.append(x); buy_y.append(y * 0.97)
            else: sell_x.append(x); sell_y.append(y * 1.03)
                
        if buy_x: fig.add_trace(go.Scatter(x=buy_x, y=buy_y, mode='markers', marker=dict(symbol='triangle-up', size=11, color='#22c55e'), name="支撐買點", hoverinfo='skip'), row=1, col=1)
        if sell_x: fig.add_trace(go.Scatter(x=sell_x, y=sell_y, mode='markers', marker=dict(symbol='triangle-down', size=11, color='#ef4444'), name="壓力賣點", hoverinfo='skip'), row=1, col=1)

        # 最極端的歷史高低點
        fig.add_hline(y=highest, line_dash="dash", line_color="#ef4444", row=1, col=1, annotation_text=f"歷史高點 {highest:.1f}", annotation_position="top right", annotation_font_color="#ef4444")
        fig.add_hline(y=lowest, line_dash="dash", line_color="#22c55e", row=1, col=1, annotation_text=f"歷史低點 {lowest:.1f}", annotation_position="bottom right", annotation_font_color="#22c55e")
