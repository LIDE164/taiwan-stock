...
def draw_professional_chart(
    ...
    fig.add_annotation(
        x=x_vals[0], y=df_view['High'].max(), text=ma_text, 
        showarrow=False, xanchor='left', yanchor='bottom',
        font=dict(color="#facc15", size=11), row=1, col=1
    )
    ...
    if show_buy_signal:
        ...
        if ai_x:
            fig.add_trace(go.Scatter(
                ...
                textfont=dict(color="#22c55e", size=10, weight="bold"),
    ...
    if len(x_vals) > 0:
        ...
        fig.add_annotation(
            x=last_date,
            y=df_view['High'].max() * 1.05,
            text=f"🤖 AI勝率 {win_rate:.1f}%",
            showarrow=False,
