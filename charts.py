# charts.py - AI機構級強化版圖表模組（箭頭防碰撞與排版優化版）
...
def detect_breakout(df_view, levels):
    ...
    for lvl in levels:
        for i in range(1, len(df_view)):
            ...
            # 抓取當日最高與最低價，用來定位箭頭避免撞到K棒
            low_p = df_view['Low'].iloc[i]
            high_p = df_view['High'].iloc[i]
            
            # 突破條件（站上 + 收盤確認）
            if c_curr > lvl and c_prev <= lvl:
                signals.append(("buy", date, low_p))
            # 跌破
            if c_curr < lvl and c_prev >= lvl:
                signals.append(("sell", date, high_p))
    return signals
...
def draw_professional_chart(df, latest_price, view_days=120, is_light_mode=False, show_buy_signal=True, show_sup_res=True, show_signals=True, buy_dates=None):
    ...
    if show_sup_res:
        ...
        for typ, x, ref_p in signals:
            if typ == "buy": 
                buy_x.append(x)
                buy_y.append(ref_p * 0.96)  # 突破箭頭距離：最低價往下偏移 4%
            else: 
                sell_x.append(x)
                sell_y.append(ref_p * 1.04) # 跌破箭頭距離：最高價往上偏移 4%
    ...
    if show_buy_signal:
        ...
        for i in range(len(df_view)):
            ...
            if row.get('ai_buy', False):
                ...
                ai_y.append(row['Low'] * 0.91) # AI星星距離：最低價往下偏移 9% (避開突破箭頭)
    ...
    fig.update_yaxes(range=[lowest * 0.82, highest * 1.15], row=1, col=1) # Y軸再拉開一點給文字空間

    # ===== 2. 成交量 =====
    # 加入透明度 opacity=0.85 提升質感
    ...
    return fig# charts.py - AI機構級強化版圖表模組（箭頭防碰撞與排版優化版）
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import pandas as pd
import numpy as np
