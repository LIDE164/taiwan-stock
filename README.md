...
The Telegram link bot accepts a Taiwan stock code, name, or `/stock <code>` from the allow-listed `TELEGRAM_CHAT_ID` and replies with a button linking directly to that stock's Streamlit analysis page. `.github/workflows/telegram_link_bot.yml` polls pending messages every five minutes—the shortest GitHub Actions schedule interval—and also supports manual dispatch. The public repository uses a standard GitHub-hosted runner and requires only the existing `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` secrets; no GCP billing account or Cloud Run service is required. The production Streamlit URL is the default, and the optional repository variable `ANALYSIS_BASE_URL` can override it after an app-domain change. The app resolves both `?stock=2330` and URL-encoded `?query=台積電`, while ambiguous names request a ticker instead of silently opening the wrong stock.
