...
After a completed ranking, the scanner sends two 1080×1400 Telegram PNGs: the overall Top-10 and a second list containing only records whose saved `Entry_Status` is `現在可執行`. The second image includes entry zone, stop, target, technical win rate, sample credibility, and an odd-lot allocation. The allocation is water-filled across the displayed names using each entry-zone upper bound plus estimated buy commission, with a total daily cap of NT$5,000; an honest empty-state image is sent when nothing qualifies. Configure `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` either as GitHub repository secrets or inside `STREAMLIT_SECRETS`. Independent delivery state is stored in `notifications/daily_top10_{date}` and `notifications/daily_executable_{date}`, so the 22:00 backup run only retries a missing image; `python scanner.py --resend-telegram` intentionally overrides both guards.
