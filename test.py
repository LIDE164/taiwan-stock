# 最後修改時間: 2026-06-29 19:00 CST
# 版本: 原版完美UI + 產業精準修復 + 左側全局過濾 + 自選股回歸 + 大戶/回測圖表
...
st.sidebar.markdown("<span style='font-size:0.85rem; color:#888;'>此過濾會即時套用於右側榜單與熱力圖</span>", unsafe_allow_html=True)
...
ind_filter_placeholder = st.sidebar.empty() # 產業選單佔位符
st.sidebar.divider()
...
# 🌟 修復產業精準度：加入強大的備援字典與 User-Agent 防護
...
def render_index_board():
    ...
    with mc1.container(border=True): st.markdown(f"<div style='text-align:center; font-size:0.85rem;'>費城半導體</div><div style='text-align:center; font-size:1.1rem; font-weight:bold; color:{'#ff3333' if sox['pct']>=0 else '#00cc00'};'>{sox['price']:,.1f}<br>{'+' if sox['pct']>0 else ''}{sox['pct']:.2f}%</div><div style='text-align:center; font-size:0.75rem; color:#888; margin-top:5px;'>🕒 {sox['time']}<br>🔗 來源: Yahoo Finance</div>", unsafe_allow_html=True)
    
    vix = macro.get('^VIX', {"price": 0, "pct": 0, "time": "無", "url": "#"})
    with mc2.container(border=True): st.markdown(f"<div style='text-align:center; font-size:0.85rem;'>VIX 恐慌指數</div><div style='text-align:center; font-size:1.1rem; font-weight:bold; color:{'#00cc00' if vix['pct']<=0 else '#ff3333'};'>{vix['price']:,.2f}<br>{'+' if vix['pct']>0 else ''}{vix['pct']:.2f}%</div><div style='text-align:center; font-size:0.75rem; color:#888; margin-top:5px;'>🕒 {vix['time']}<br>🔗 來源: Yahoo Finance</div>", unsafe_allow_html=True)
    
    jpy = macro.get('JPY=X', {"price": 0, "pct": 0, "time": "無", "url": "#"})
    with mc3.container(border=True): st.markdown(f"<div style='text-align:center; font-size:0.85rem;'>日圓動向(USD/JPY)</div><div style='text-align:center; font-size:1.1rem; font-weight:bold; color:#ffcc00;'>{jpy['price']:,.2f}<br>{'央行趨緩' if jpy['pct']>0 else '升息撤資警戒'}</div><div style='text-align:center; font-size:0.75rem; color:#888; margin-top:5px;'>🕒 {jpy['time']}<br>🔗 來源: Yahoo Finance</div>", unsafe_allow_html=True)
...
def draw_professional_chart(df, ticker_name, latest_price, view_days, is_light_mode, show_buy_signal=False, show_sup_res=False, show_signals=True):
    ...
    fig.add_annotation(text="📊 資料來源: Fugle富果 API / FinMind", xref="paper", yref="paper", x=1.0, y=-0.05, showarrow=False, font=dict(size=12, color=text_c))
    return fig
...
if st.session_state.page == "home":
    ...
    if scan_results:
        ...
        for _, r in df_disp.iterrows():
            ...
            if st.button(f"▪️ {r['代號']} {r['名稱']} {trend_icon}{r['收盤價']}({sign}{r['漲跌幅']:.2f}%) | 產業:{r['產業']} | {score_icon}{tag_display}", key=f"btn_scan_{r['ticker_raw']}", use_container_width=True):
            ...
            if df_chart is not None and len(df_chart) >= 5:
                ...
                with col_main:
                    ...
                    with ec2.container(border=True):
                        ...
                        fig_eq.update_layout(title="累積獲利曲線 (初始 10 萬)", height=200, margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor=app_bg, plot_bgcolor=app_bg)
                    ...
                    if data['成交量'] > data['5日均量'] * 1.1 and data.get('漲跌',0) > 0: t_bull.append(f"🔥 <span style='color:#ff3333; font-weight:bold;'>量價確認：今日量能放大大於5日均量，主力點火信號明確。</span>")
                    if not t_bull: t_bull.append(f"⚪ 股價表現偏弱或震盪整理，動能不顯著。")
                    
                    t_html = f"<div style='border: 1px solid {b_col}; border-radius: 8px; padding: 15px; margin-bottom: 15px; background-color: {card_bg};'><h4 style='color: #00ccff; margin-top: 0;'>📈 技術動能 <span style='font-size:0.8rem; color:#888;'>[來源: Fugle富果]</span></h4><ul style='color:{t_text_c}'>"
                    ...
                    fig = draw_professional_chart(df_slice, target, data['收盤價'], st.session_state.view_days, is_light_mode, st.session_state.get('tgl_buy', True), st.session_state.get('tgl_sup', True), st.session_state.get('tgl_sig', True))
