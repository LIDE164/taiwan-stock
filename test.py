...
def get_decision_score(data, fund_data, inst_data=None, mode='short_term'):
    ...
    if eps_f > 0: 
        if mode == 'long_term':
            sc += 4; rs.append("✅ 基本面獲利 (長線護城河)")
        else:
            sc += 2; rs.append("✅ 基本面獲利")
    elif eps_f < 0:
        if mode == 'long_term':
            sc -= 5; rs.append("⚠️ 基本面虧損 (嚴防長線變壁紙)")
        else:
            sc -= 1; rs.append("⚠️ 基本面虧損")
...
def generate_comprehensive_analysis(data, inst_data, sc, f_data, market_today="", market_tmr="", is_light_mode=False, mode="short_term"):
    ...
    mode_label = "中長期波段" if mode == "long_term" else "短線動能"
    ...
    elif data['J值'] > 80 and mode == 'short_term': tech_bullets.append(f"⚠️ <span style='color:#00cc00;'><b>KDJ 高檔過熱</b>：J 值高達 {data['J值']}，短線過熱步入超買區。</span>")
    ...
    elif data['成交量'] < data['5日均量'] and mode == 'short_term': tech_bullets.append(f"⚠️ <span style='color:#00cc00;'>量能警訊：反彈或震盪中「量能萎縮」，需防範缺乏買盤支撐的虛假反彈。</span>")
    ...
    try: 
        ...
        if eps_f > 0 and pe_f < 15:
            fund_res = "🔥 價值嚴重低估！具備實質獲利支撐，且本益比在合理水位之下，長線防護力極高。"
        elif eps_f > 0 and pe_f < 25:
    ...
    # 🚀 建倉區間與最終總結邏輯 (根據長短線給出不同建議)
    current_p = data['收盤價']
    if mode == 'long_term':
        lower_bound = data['20MA'] if current_p > data['20MA'] else data['60MA']
    else:
    ...
    if sc >= 5: 
        ...
        v_a = f"✅ <b>進場判斷：強烈買進 ({mode_label})</b><br>動能、量能、型態/價值三道關卡全數確認過關！<br>📌 建議建倉區間：現價 ({current_p:.2f}) ~ 逢低 {range_min:.2f} 之間分批加碼。"
    elif sc >= 2: 
        v_t, v_c = "🟡 A級機會：偏多試單", "#ffcc00"
        v_a = f"✅ <b>進場判斷：分批試單 ({mode_label})</b><br>滿足跌深超賣或趨勢支撐條件，動能防護及格。<br>📌 建議建倉區間：現價 ({current_p:.2f}) ~ 逢低 {range_min:.2f} 之間佈局，跌破前波低點嚴格停損。"
    elif sc >= -1: 
        ...
        v_a = f"⛔ <b>進場判斷：絕對空手 ({mode_label})</b><br>量能、動能完全走空，或基本面呈現衰退警訊。強烈建議空手觀望，切勿拿資金接落下的飛刀。<br>📌 目前現價 ({current_p:.2f})，破底風險極高。"
...
if st.session_state.page == "home":
    ...
    st.markdown(f"<h3 style='margin-top: 15px;'>🎯 策略條件篩選 ({'短線動能模式' if st.session_state.strategy_mode == 'short_term' else '中長波段模式'})</h3>", unsafe_allow_html=True)
    ...
    if df_chart is not None and len(df_slice) >= 5:
        ...
        with col_main_view:
            ...
            st.markdown(f"##### 💡 近一個月歷史買點回測與趨勢分析 ({'短線動能模式' if st.session_state.strategy_mode == 'short_term' else '中長波段模式'})")
            ...
            # 💡 在這裡去除了所有的多行字串縮排，避免 Streamlit 誤判
