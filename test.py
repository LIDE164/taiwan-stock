...
def get_futures_quote():
    ...
    # 第二層：Yahoo Finance 歷史資料 (繞過 Query 封鎖，直接用 yfinance 取最後1分K)
    try:
        tk = yf.Ticker("TWF=F")
...
# 🎯 將掃描快取函數更名為 v2，徹底打破 Streamlit KeyError 舊快取魔咒
@st.cache_data(ttl=180, show_spinner=False)
def get_global_scan_results_v2(pool_tuple):
    scan_results = []
...
if st.session_state.page == "home":
    ...
    with st.spinner("🚀 大腦背景資料庫存取中..."):
        scan_results = get_global_scan_results_v2(pool)
            
    if scan_results:
        pd.set_option('future.no_silent_downcasting', True)
        df_results = pd.DataFrame(scan_results)
        
        # 🎯 安全防護：確保 DataFrame 中絕對有 Vol_Ratio 欄位
        if 'Vol_Ratio' not in df_results.columns:
            df_results['Vol_Ratio'] = df_results['成交量'] / df_results['5日均量'].replace(0, 1)
