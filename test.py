...
# 🎯 終極防呆：確保 Session State 絕對存在，杜絕 AttributeError
...
def get_stock_name(ticker):
    ...
    if ticker_str in CURRENT_STOCK_NAMES and CURRENT_STOCK_NAMES[ticker_str]: name = CURRENT_STOCK_NAMES[ticker_str]
        ...
        if html_name: STOCK_NAMES[ticker_str] = html_name; name = html_name
...
def fetch_live_tick_data(ticker):
    ...
    try:
        ...
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'Referer': f'https://www.wantgoo.com/stock/{base_ticker}', 'Accept': 'application/json'}
        ...
        if res.status_code == 200:
            ...
            if raw_ticks:
                ...
                for t in raw_ticks[:30]: 
                    ...
                    processed_ticks.append({"時間": t.get('time', ''), "價格": float(t.get('price', 0)), "現量": t_vol, "屬性": "外盤(買進)" if t_type=='buy' else ("內盤(賣出)" if t_type=='sell' else "平盤")})
    ...
    try:
        ...
        if not df.empty:
            ...
            for idx, row in df.iterrows():
                ...
                if is_buy: ask_vol += t_vol
                else: bid_vol += t_vol
                processed_ticks.append({"時間": idx.strftime('%H:%M:%S'), "價格": round(float(row['Close']), 2), "現量": t_vol, "屬性": "外盤(買進)" if is_buy else "內盤(賣出)"})
            total_v = ask_vol + bid_vol
            return {"ticks": processed_ticks, "ask_ratio": round((ask_vol / total_v) * 100, 1) if total_v > 0 else 50.0, "bid_ratio": round((bid_vol / total_v) * 100, 1) if total_v > 0 else 50.0, "total_volume": total_v}
    except: pass
    return fallback
...
def get_decision_score(data, fund_data, inst_data=None, tick_data=None):
    ...
    if vol > vol_5ma * 2 and is_red_k and data['BIAS'] < 5:
        sc += 2; rs.append(f"🔥 量能點火 (成交量達均量 {round(vol/max(vol_5ma, 1), 1)} 倍，主力資金進駐)"); data['量能點火'] = True
    elif vol > vol_5ma * 1.2: sc += 1; rs.append(f"✅ 量能溫和放大 (達均量 {round(vol/max(vol_5ma, 1), 1)} 倍)")
...
def analyze_today(df, ticker_number, inst_data=None, tick_data=None):
    ...
    try:
        ...
        is_red_engulfing, is_black_engulfing, recent_7_red, is_support_pullback, is_resistance_rejection = False, False, False, False, False
    ...
    data = {
        ...
        "成交量": vol, "5日均量": vol_5ma, "Vol_Ratio": round(vol / max(1, vol_5ma), 2),
    ...
    try:
        ...
        with st.container(border=True):
            ...
            with col1:
                ...
                st.markdown(f"<div style='text-align: center; font-size: 2.2rem; font-weight: 900; color: {twii_color}; margin: 0;'>{twii_close:,.0f}</div>", unsafe_allow_html=True)
                ...
                st.markdown(f"<div style='text-align: center; font-size: 0.85rem; color: #888;'>🕒 {twii_time_str}</div>", unsafe_allow_html=True)
            with col3:
...
def generate_comprehensive_analysis(data, inst_data, sc, market_today="", market_tmr=""):
    ...
    if market_today and market_tmr:
        ...
        if "多" in market_tmr_clean or "高" in market_tmr_clean: analysis_bullets.append(f"🔥 <span style='color:#ff3333; font-weight:bold;'>大盤盤勢導航：當前【{market_today_clean}】，預測次一交易日【{market_tmr_clean}】，大環境偏多有利個股發揮。</span>")
        elif "空" in market_tmr_clean or "低" in market_tmr_clean: analysis_bullets.append(f"⚠️ <span style='color:#00cc00;'>大盤盤勢導航：當前【{market_today_clean}】，預測次一交易日【{market_tmr_clean}】，大環境不佳需防範系統性風險。</span>")
        else: analysis_bullets.append(f"⚪ <b>大盤盤勢導航</b>：當前【{market_today_clean}】，預測次一交易日【{market_tmr_clean}】，大環境震盪，個股表現分歧。")
