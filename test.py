...
# --- Google Finance 大盤讀取防護 ---
import requests
from bs4 import BeautifulSoup

@st.cache_data(ttl=60) 
def get_google_finance_price(query_symbol):
    try:
        url = f"https://www.google.com/finance/quote/{query_symbol}"
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
        res = requests.get(url, headers=headers, timeout=5)
        soup = BeautifulSoup(res.text, 'html.parser')
        
        price_div = soup.find('div', {'class': 'YMlKec fxKbKc'})
        if not price_div: return None, None
        ...
        if change_div:
            ...
            if 'JwB6zf' in change_div.parent.get('class', []): 
                change = -abs(change)
        return price, change
...
def render_index_board():
    ...
    # --- 1. 使用 Google Finance 抓取大盤，設定正確的代號 ---
    twii_close, twii_change = get_google_finance_price("TAIEX:INDEXTWSE")
    
    # 備用防護：若 Google 阻擋，則用台灣50 (0050) 計算大盤走勢代表
    ...
    with st.container(border=True):
        ...
        with col1:
            st.markdown(f"<div style='text-align: center; font-size: 1.1rem; font-weight: bold;'><a href='https://www.google.com/finance/quote/TAIEX:INDEXTWSE' target='_blank' style='color:#ccc; text-decoration:none;'>台灣加權指數 🔗</a></div>", unsafe_allow_html=True)
        ...
        with col2:
            ...
            # --- 2. 修復首頁新聞排版位置，置於預測模型下方 ---
            news_items = get_real_news("台股", "大盤")
            us_news_html = "<div style='margin-top: 15px; border-top: 1px solid #444; padding-top: 10px; text-align: left;'><span style='font-size:0.95rem; font-weight:bold; color:#ffcc00;'>📰 財經焦點新聞：</span><br>"
            if news_items:
