# 最後修改時間: 2026-07-06 (滿血回歸版：三大面解析恢復 + 首頁華麗卡片 + 產業別雙重備援)
...
# 🚀 核心計算與抓取模組
# ==========================================
# 大幅擴充產業字典，對應 Yahoo Finance 各種英文
ENG_TO_TW_INDUSTRY = {
...
def get_fundamental_and_industry_data(ticker_number, current_price=0):
    ...
    # 雙重備援：先透過 Yahoo 抓取，如果失敗再透過鉅亨網 API 抓取產業
    try:
        ...
        if re.search(r'[a-zA-Z]', ind_temp) or ind_temp == "一般產業":
            ...
            if 'data' in res_cnyes and 'categoryName' in res_cnyes['data']:
                ind = res_cnyes['data']['categoryName']
        else:
            ind = ind_temp
    ...
    # 若 EPS 也抓不到，補強鉅亨網 API
    if eps_val == "無":
        try:
            res_api = requests.get(f"https://ws.cnyes.com/twstock/api/v1/company/profile/{base_ticker}", timeout=3).json()
            if 'data' in res_api and 'eps' in res_api['data']: 
                eps_val = f"{float(res_api['data']['eps']):.2f}"
                if ind == "一般產業" and 'categoryName' in res_api['data']: ind = res_api['data']['categoryName']
        except: pass
...
def generate_comprehensive_analysis(data, inst_data, sc, f_data, is_light_mode=False):
    ...
    else: tech_bullets.append(f"⚠️ <span style='color:#facc15; font-weight:bold;'>ADX 趨勢指標 ({adx})：低於 25，正處於橫盤震盪，較容易假突破。</span>")
    if roc > 10: tech_bullets.append(f"🔥 <span style='color:#ef4444; font-weight:bold;'>強勢股濾網 (近月漲幅 {roc}%)：大幅打敗大盤，屬於主流強勢標的。</span>")
