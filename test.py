# 最後修改時間: 2026-07-06 (升級：模擬績效儀表板 2.0 + 策略回測實驗室)
...
def get_decision_score(data, fund_data, inst_data=None):
    ...
    if data.get('訊號', False): 
        if data.get('ADX',0) >= 25: sc+=3; rs.append(f"✅ 穩在月線上且KDJ超賣 (趨勢明確)")
        else: sc+=1; rs.append(f"⚠️ 穩在月線上 (動能稍弱)")
    if data.get('收盤價', 0) <= data.get('BB_DN', 0) * 1.02: sc+=2; rs.append("✅ 觸及布林下軌支撐")
    if data.get('BIAS', 0) < -5: sc+=1; rs.append("✅ 負乖離過大")
    if data.get('ROC_20',0) > 10: sc+=2; rs.append(f"🔥 近月強勢表現亮眼")
    if data.get('MoM', 0) > 0 and data.get('YoY', 0) > 0: sc+=3; rs.append(f"🔥 月營收雙增")
    try: eps_f = float(str(fund_data['EPS']).replace(',', ''))
    except: eps_f = 0.0
    if eps_f > 0: sc+=2; rs.append("✅ 歷史 EPS 獲利體質")
    if data.get('MACD柱', 0) > data.get('前日MACD柱', -999): sc+=2; rs.append("✅ MACD多方波段動能增強")
    else: sc-=3; rs.append("⚠️ MACD空方動能正在衰退")
    if data.get('紅吞', False): sc+=4 if data.get('ADX',0) >= 25 else 1; rs.append("🔥 出現「紅吞」反轉")
    if data.get('回測有撐', False): sc+=2; rs.append("🔥 帶量長下影線 (支撐防守)")
    if data.get('J值', 0) >= 80: sc-=3; rs.append("⚠️ KDJ高檔過熱")
    if data.get('收盤價', 0) < data.get('20MA', 0): sc-=2; rs.append("⚠️ 跌破月線轉為弱勢")
    return sc, rs

def get_dynamic_theme(ticker, industry):
    ind = str(industry).strip() if pd.notna(industry) else "一般產業"
...
def analyze_today(df, ticker_number, inst_data=None, is_light_mode=False, pre_fund=None):
    ...
    data = {
        ...
        "20MA": round(t.get('20MA', t_close), 2), "BB_UP": round(t.get('BB_UP', t_close), 2), "BB_DN": round(t.get('BB_DN', t_close), 2), 
        ...
        "MoM": mom, "YoY": yoy, "ForeignNet10d": f_net_10d, "TrustNet10d": t_net_10d, "DealerNet10d": d_net_10d, 
        "訊號": (t_close > t.get('20MA', 0)) and (t_close < t.get('5MA', 9999)) and (t.get('J', 50) < 20),
        "紅吞": bool(red_mask.iloc[-1]), "黑吞": bool(black_mask.iloc[-1]),
        "回測有撐": (lower_shadow > body * 1.5) and (lower_shadow / total_range > 0.4) and (t_low < p_close),
        "反彈遇壓": (upper_shadow > body * 1.5) and (upper_shadow / total_range > 0.4),
...
def calculate_historical_winrate_interactive(df_slice, target_mult, stop_mult):
    ...
    for idx in range(len(recent_90)):
        ...
        if len(temp_df) >= 14:
            ...
            p = temp_df.iloc[-2]
            sc = 0
            if (t['Close'] > t.get('20MA', 0)) and (t['Close'] < t.get('5MA', 9999)): sc+=1
            if t['Close'] <= t.get('BB_DN', t['Close']) * 1.02: sc+=2
            if t.get('MACD_Hist', 0) > p.get('MACD_Hist', 0): sc+=2
            else: sc-=3
            if t.get('J', 50) >= 80: sc-=3
            if t['Close'] < t.get('20MA', 0): sc-=2
            red_mask = (p['Open'] > p['Close']) and (t['Close'] > t['Open']) and (t['Close'] > p['Open']) and (t['Open'] < p['Close'])
            if red_mask: sc+=2
            ...
            if sc >= 2:
                ...
                future_df = df_slice.iloc[actual_idx + 1 : actual_idx + 10] # 觀察未來10天
                if len(future_df) > 0:
                    closed_signals += 1
...
def generate_comprehensive_analysis(data, sc, is_light_mode=False):
    ...
    for b in data.get('Reasons', []): tech_html += f"<li style='margin-bottom:6px;'>{b}</li>"
    tech_html += f"</ul></div>"
    return tech_html

def draw_professional_chart(df, ticker_name, latest_price, view_days, is_light_mode, show_buy_signal=False, show_sup_res=False, show_signals=True, buy_dates=[]):
    ...
    fig.add_trace(go.Candlestick(x=x_vals, open=df_view['Open'], high=df_view['High'], low=df_view['Low'], close=df_view['Close'], increasing_line_color='#ef4444', decreasing_line_color='#22c55e'), row=1, col=1)
    ...
    if show_sup_res:
        fig.add_hline(y=df_view['High'].max(), line_dash="dash", line_color="#ef4444", row=1, col=1, annotation_text=f"壓力 {df_view['High'].max():.2f}", annotation_position="top right")
        fig.add_hline(y=df_view['Low'].min(), line_dash="dash", line_color="#22c55e", row=1, col=1, annotation_text=f"支撐 {df_view['Low'].min():.2f}", annotation_position="bottom right")

    if show_buy_signal and buy_dates:
        buy_x, buy_y = [], []
        for d in buy_dates:
            if d in df_view.index:
                buy_x.append(d.strftime('%Y-%m-%d')); buy_y.append(df_view['Low'].iloc[df_view.index.get_loc(d)] * 0.90)
        if buy_x: fig.add_trace(go.Scatter(x=buy_x, y=buy_y, mode='markers+text', marker=dict(symbol='triangle-up', size=14, color='#34d399'), text=["買"]*len(buy_x), textposition="bottom center", textfont=dict(color="#34d399", size=11, weight="bold"), hoverinfo='skip'), row=1, col=1)
    ...
    fig.add_trace(go.Bar(x=x_vals, y=df_view.get('MACD_Hist', 0), marker_color=macd_colors), row=3, col=1)
    fig.add_trace(go.Scatter(x=x_vals, y=df_view.get('K', 50), line=dict(color=line_k)), row=4, col=1)
    fig.add_trace(go.Scatter(x=x_vals, y=df_view.get('D', 50), line=dict(color=line_d)), row=4, col=1)

    fig.update_xaxes(type='category', nticks=15, showgrid=True, gridcolor=grid_c)
...
def generate_cards_html(df_disp, is_intraday):
    ...
    for _, r in df_disp.iterrows():
        p_col = "#ef4444" if r.get('漲跌', 0) >= 0 else "#22c55e"
        score = r.get('Intraday_Score', 50) if is_intraday else r.get('Score', 0)
        s_col = "#ef4444" if score >= (80 if is_intraday else 5) else ("#facc15" if score >= (60 if is_intraday else 2) else "#22c55e")
        rating = "動能" if is_intraday else r.get('評級', '觀望').replace('🟢 ', '').replace('🟡 ', '').replace('⚪ ', '')
        ...
        cards_html += f"<div style='display: flex; align-items: center; gap: 12px;'><div style='width: 50px; height: 50px; border-radius: 50%; background: #1e293b; display: flex; flex-direction: column; align-items: center; justify-content: center;'><span style='color: {s_col}; font-weight: 800; font-size: 1.2rem;'>{score}</span></div>"
        cards_html += f"<a href='/?stock={r.get('代號', '')}' target='_self' style='text-decoration:none;'><div style='color: #f8fafc; font-weight: bold; font-size: 1.15rem;'>{r.get('名稱', '')} <span style='font-size: 0.8rem; color: #64748b;'>{r.get('代號', '')}</span></div></a></div>"
        cards_html += f"<div style='text-align: right;'><div style='color: {p_col}; font-weight: 800; font-size: 1.2rem;'>{r.get('收盤價', 0):.1f}</div><div style='color: {p_col}; font-size: 0.75rem;'>{'+' if r.get('漲跌',0)>0 else ''}{r.get('漲跌幅', 0)}%</div></div></div>"
        
        if is_intraday:
            cards_html += f"<div style='display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; font-size: 0.75rem;'><div style='color: #64748b;'>預估量能 <span style='color: #e2e8f0; font-weight: bold;'>{r.get('Est_Vol_Ratio', 1):.1f}x</span></div><div style='color: #64748b;'>主力動向 <span style='color: #ef4444; font-weight: bold;'>{r.get('Flow', '')}</span></div><div style='color: #64748b;'>歷史勝率 <span style='color: #facc15; font-weight: bold;'>{r.get('WinRate', 0):.1f}%</span></div></div>"
        else:
            cards_html += f"<div style='display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; font-size: 0.75rem;'><div style='color: #64748b;'>歷史勝率 <span style='color: #facc15; font-weight: bold;'>{r.get('WinRate', 0):.1f}%</span></div><div style='color: #64748b;'>法人淨買 <span style='color: #e2e8f0; font-weight: bold;'>{r.get('Whale_Net', 0):,}</span></div><div style='color: #64748b;'>主力特徵 <span style='color: #ef4444; font-weight: bold;'>{r.get('Feature', '')}</span></div></div>"
        cards_html += f"</div>"
    return cards_html
...
if st.session_state.page == "home":
    ...
    if st.session_state.scan_results:
        ...
        if not df_results.empty: df_disp = df_results.sort_values(by=['Intraday_Score' if is_intraday else 'Score', '漲跌幅'], ascending=[False, False]).head(30)
        ...
        if df_disp.empty: st.info("此條件下暫無符合條件的標的。")
...
# ==========================================
    ...
    if not orders: st.info("目前沒有模擬下單紀錄，去解析頁面建立你的第一筆策略單吧！")
        ...
        for order in orders:
            ...
            total_cost += order['buy_price'] * 1000 # 假設每筆買一張(1000股)
        ...
        <div style='display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-bottom: 25px;'>
            ...
            <div style='background-color: rgba(30,41,59,0.5); padding: 15px; border-radius: 10px; text-align: center; border: 1px solid #1e293b;'>
                ...
                <div style='color: #64748b; font-size: 0.9rem;'>(賺: {wins} 筆 / 總: {len(orders)} 筆)</div>
            </div>
            <div style='background-color: rgba(30,41,59,0.5); padding: 15px; border-radius: 10px; text-align: center; border: 1px solid #1e293b;'>
                ...
                <div style='color: #64748b; font-size: 0.9rem;'>(假設每檔固定 1 張)</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
# ==========================================
    ...
    if df_chart is not None and len(df_chart) >= 14:
        ...
        with st.expander("⚙️ 調整停利/停損參數 (預設風報比 1:1.5)", expanded=True):
            ...
            with s_col1:
                atr_stop_mult = st.slider("🛑 停損區間 (ATR倍數)", min_value=0.5, max_value=3.0, value=1.0, step=0.1, help="數值越小，停損越緊；數值越大，容忍震盪空間越大。")
            with s_col2:
                atr_target_mult = st.slider("💰 停利目標 (ATR倍數)", min_value=0.5, max_value=5.0, value=1.5, step=0.1, help="數值越大，賺取波段利潤越多，但達標機率可能降低。")
        ...
        <div style="border: 2px solid {v_c}; border-radius: 10px; padding: 20px; margin-bottom: 20px; background-color: #0b1120;">
            ...
            <div style="background-color: rgba(30,41,59,0.5); padding: 15px; border-radius: 8px; border-left: 5px solid {v_c}; margin-bottom:20px;">
                <p style="font-size: 1.05rem; color: #f8fafc; margin: 0; line-height: 1.6;">
                    ✅ <b>自訂策略執行規劃</b><br>
                    合理停利目標：<b style='color:#ef4444;'>{data['ATR_Target']}</b> 元<br>
                    嚴格停損防守：<b style='color:#22c55e;'>{data['ATR_Stop']}</b> 元
                </p>
            </div>
            {generate_comprehensive_analysis(data, sc, is_light_mode)}
        ...
        fig = draw_professional_chart(df_slice, target, data['收盤價'], st.session_state.view_days, is_light_mode, current_show_buy, current_show_sup, current_show_signals, buy_dates=buy_dates)
